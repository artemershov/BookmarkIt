...
app.directive('bookmarkImage', ['$filter', 'blobStorage', ($filter, blobStorage) => ({
  restrict: 'EA',
  replace: true,
  scope: {
    width: '@',
  ...
  link: ($scope, element) => {
    ...
    const drawText = (el, width, height, text, font, color) => {
      const c = el.getContext('2d');
      return document.fonts.load(font.style + ' ' + font.size + 'px ' + font.name).then(() => {
        // Background
    ...
    const draw = type => {
      ...
      switch (type) {
        case 'picture': return url   => drawPicture(el, w, h, url);
        case 'icon':    return param => drawIcon(el, w, h, ...param);
        case 'text':    return text  => drawUrl(el, w, h, text);
        case 'blob':    return blob  => drawBlob(el, w, h, blob);
      }
    };
