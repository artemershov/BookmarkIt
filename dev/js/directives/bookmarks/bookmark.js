...
app.directive('bookmark', [
  ...
  (ajaxAPI, $filter, messages) => ({
    ...
    link: ($scope, element, attrs) => {
      ...
      const actions = {
          ...
          ajaxAPI.bookmarks.edit(b.id, { star: +!b.star }).then(r => {
            if (r == 1) {
              ...
              if (b.star) {
                notify
                  .update({
                    text: messages.bookmarks.success.fav,
                    icon: 'fa-star',
                  })
                  .show();
              } else {
                notify
                  .update({
                    text: messages.bookmarks.success.unfav,
                    icon: 'fa-star-o',
                  })
                  .show();
              }
              ...
              notify
                .update({ text: messages.bookmarks.error, icon: 'fa-warning' })
                .show();
            }
          });
        },
        ...
        reorder: (a, f, t) => {
          ...
          // If changes
          if (f !== t) {
            let gap = $filter('bookmarksReorderFilter')(
              $scope.bookmarks,
              a[f].pos,
              a[t].pos
            );
            ...
            ajaxAPI.bookmarks.reorder(modify).then(r => {
              if (r) {
                notify
                  .update({
                    text: messages.bookmarks.success.reorder,
                    icon: 'fa-check',
                  })
                  .show();
              } else {
        ...
        delete: id =>
          $scope.confirm.open({
            ...
            agree: () =>
              ajaxAPI.bookmarks.delete(id).then(r => {
                if (r == 1) {
                  ...
                  notify
                    .update({
                      text: messages.bookmarks.success.delete,
                      icon: 'fa-trash-o',
                    })
                    .show();
                } else {
    ...
    templateUrl: (element, attrs) =>
      'directives/bookmark/view/' + attrs.type + '.html',
  }),
]);
