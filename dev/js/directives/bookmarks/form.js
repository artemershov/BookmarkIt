...
app.directive('bookmarkForm', [
  ...
  (ajaxAPI, route, $filter, messages) => ({
      ...
      switch (attrs.type) {
              ...
              if (add.data.link && add.data.gid) {
                ...
                ajaxAPI.bookmarks.add(add.data).then(data => {
                  if (!(data == 0 || data == undefined)) {
                    ...
                    notify
                      .update({
                        text: messages.bookmarks.success.add,
                        icon: 'fa-plus-circle',
                      })
                      .show();
                  } else {
                ...
                let m = '';
                if (!add.data.link && !add.data.gid)
                  m = messages.bookmarks.validator.both;
              ...
              submit: () => {
                ...
                ajaxAPI.bookmarks.edit(edit.item.id, edit.copy).then(r => {
                  ...
                  if (r == 1) {
                    ...
                    notify
                      .update({
                        text: messages.bookmarks.success.edit,
                        icon: 'fa-check',
                      })
                      .show();
                    edit.close();
        ...
        case 'batch': {
          const batch = {
            data: {
              selected: () =>
                $filter('filter')($scope.filteredBookmarks(), { batch: true }),
              agree: false,
              action: null,
              gid: null,
            },
            ...
            submit: () => {
              if (batch.data.action == 'delete' && !batch.data.agree) {
                notify
                  .update({
                    text: messages.bookmarks.validator.delete,
                    icon: 'fa-warning',
                  })
                  .show();
              } else {
                  ...
                  if (r) {
                    switch (batch.data.action) {
                      case 'move': {
                        ...
                        notify
                          .update({
                            text: messages.bookmarks.batch.move,
                            icon: 'fa-folder',
                          })
                          .show();
                        break;
                      ...
                      case 'delete': {
                        ...
                        notify
                          .update({
                            text: messages.bookmarks.batch.delete,
                            icon: 'fa-trash-o',
                          })
                          .show();
                        break;
    ...
    templateUrl: (element, attrs) =>
      'directives/bookmark/form/' + attrs.type + '.html',
  }),
]);
